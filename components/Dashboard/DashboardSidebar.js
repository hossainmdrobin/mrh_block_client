...
const DashboardSidebar = () => {
    ...
    return (
        <div className='relative top-0 z-10'>
            ...
            <div onClick={()=>setisOpen(!isOpen)} class={isOpen?"cursor-pointer flex justify-center items-center top-4 rounded-full fixed left-64 w-10 h-10 bg-yellow-300":"cursor-pointer flex justify-center items-center top-4 rounded-full fixed left-2 w-10 h-10 bg-yellow-300"}><DotsHorizontalIcon className='h-6 text-white' /></div>
            <div onClick={()=> router.push('/')} class={isOpen?"cursor-pointer flex justify-center items-center rounded-full fixed top-20 left-64 bg-red-600 w-10 h-10":"cursor-pointer flex justify-center items-center rounded-full fixed top-20 left-2 bg-red-600 w-10 h-10"}><HomeIcon className='h-8 text-white' /></div>
        </div>
    );
}
